...
class _StudentExaminedPatientsPageState extends State<StudentExaminedPatientsPage> {
  ...
  final String _baseApiUrl = 'http://localhost:3000';
  final String _studentExaminationsApi = 'http://localhost:3000/student-examinations';
  final String _xrayImagesApi = 'http://localhost:3000/xray-images/patient'; // ✅ NEW: X-ray images API
  ...
  final Map<String, String> _englishTranslations = {
    ...
    'xray_images': 'X-ray Images', // ✅ NEW
    'no_xray_images': 'No X-ray images available', // ✅ NEW
    'xray_type': 'X-ray Type', // ✅ NEW
    'uploaded_by': 'Uploaded By', // ✅ NEW
    'uploaded_at': 'Uploaded At', // ✅ NEW
  };
  ...
  void _initializeStudentInfo() {
    setState(() {
      ...
      _allowedFeatures = widget.userAllowedFeatures ?? ['examined_patients'];
    });
  }
    ...
    try {
      ...
      if (!idImageUrl.startsWith('http')) {
        ...
        if (idImageUrl.startsWith('/')) {
          idImageUrl = 'http://localhost:3000$idImageUrl';
  ...
  Widget _buildAllPatientImages(Map<String, dynamic> patient) {
    ...
    // Check for Patient Image
    if (_isValidImageUrl(patient['IMAGE']) || _isValidImageUrl(patient['image'])) {
      final url = patient['IMAGE'] ?? patient['image'];
      availableImages.add({'title': 'Patient Photo', 'url': url.toString()});
    }
  ...
  Widget _buildSidebar() {
    return StudentSidebar(
      ...
      allowedFeatures: _allowedFeatures ?? ['examined_patients'],
    );
  }
