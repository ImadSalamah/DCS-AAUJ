...
class _RadiologyReportPageState extends State<RadiologyReportPage> {
  ...
  Future<void> _fetchReport() async {
    ...
    try {
      ...
      final url = 'http://localhost:3000/xray_custom_report?startDate=$startDate&endDate=$endDate';
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          reportData = data as Map<dynamic, dynamic>? ?? {};
          isLoading = false;
          errorMsg = reportData!.isEmpty ? 'لا يوجد بيانات للفترة المختارة' : null;
        });
      } else {
        setState(() {
          reportData = {};
          errorMsg = 'خطأ في السيرفر (${response.statusCode})';
          isLoading = false;
        });
      }
