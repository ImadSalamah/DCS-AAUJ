...
class _XrayRequestListPageState extends State<XrayRequestListPage> {
  ...
  Future<void> _loadUserData() async {
    try {
      final response = await http.get(Uri.parse('http://localhost:3000/radiology/profile'));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        setState(() {
          userName = data['fullName'] ?? 'فني الأشعة';
  ...
  Future<void> _loadXrayWaitingPatients() async {
    ...
    try {
      final response = await http.get(Uri.parse('http://localhost:3000/xray_requests'));
  ...
  void _applyFilters() {
    ...
    if (_selectedXrayTypeFilter != 'all') {
      filtered = filtered.where((p) => p['xray_type'] == _selectedXrayTypeFilter).toList();
    }
  ...
  Future<void> _markRequestAsCompleted(Map<String, dynamic> patient) async {
    try {
      final response = await http.put(
        Uri.parse('http://localhost:3000/xray_requests/${patient['request_id']}/status'),
                        ...
                        Row(
                          children: [
                            ...
                            if (patient['status'] != 'completed')
                              ElevatedButton.icon(
                                onPressed: () => _markRequestAsCompleted(patient),
