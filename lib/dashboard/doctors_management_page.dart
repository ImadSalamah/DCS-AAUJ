...
class _DoctorsManagementPageState extends State<DoctorsManagementPage> {
  ...
  final Map<String, IconData> featureIcons = {
    ...
    'assign_patients_to_students': Icons.assignment_ind,
  };
  ...
  final Map<String, Color> featureColors = {
    ...
    'assign_patients_to_students': Colors.indigo,
  };
  ...
  Future<void> updateDoctorType(String doctorId, String type) async {
    try {
      final response = await http.put(
        Uri.parse('http://localhost:3000/doctors/$doctorId/type'),
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${_tr('connection_error')}: ${response.statusCode}')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('${_tr('connection_error')}: $e')),
      );
    }
  }
  ...
  void _fetchSingleDoctorFeatures(String doctorId) async {
    ...
    try {
      final resp = await http.get(Uri.parse('http://localhost:3000/doctors/$doctorId'));
  ...
  Future<void> _saveSingleDoctorFeatures() async {
    ...
    try {
      final response = await http.put(
        Uri.parse('http://localhost:3000/doctors/$doctorId/features'),
        headers: {'Content-Type': 'application/json'},
        body: json.encode({
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${_tr('connection_error')}: ${response.statusCode} - ${response.body}')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _saveMultipleDoctorsFeatures() async {
    ...
    try {
      ...
      final response = await http.put(
        Uri.parse('http://localhost:3000/doctors/batch/features'),
        headers: {
  ...
  Future<void> _tryAlternativeEndpoint(Map<String, dynamic> requestData) async {
    try {
      final response = await http.put(
        Uri.parse('http://localhost:3000/doctors/batch/features-simple'),
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
        },
      ...
      if (response.statusCode == 200) {
        ...
        String errorMessage = '${_tr('connection_error')}: ${response.statusCode}';
        try {
          final errorData = json.decode(response.body);
          errorMessage = errorData['error'] ?? errorData['message'] ?? errorMessage;
        } catch (_) {
          errorMessage = '${_tr('connection_error')}: ${response.body}';
        }
