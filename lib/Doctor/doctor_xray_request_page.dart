...
class _DoctorXrayRequestPageState extends State<DoctorXrayRequestPage> {
  ...
  Future<void> _loadDoctorInfo() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('http://localhost:3000/doctors/$_currentDoctorId')
      );
  ...
  Future<void> _loadStudents() async {
    try {
      ...
      final response = await http.get(Uri.parse('http://localhost:3000/students-with-users'));
  ...
  Future<void> _loadAllowedFeatures() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          allowedFeatures = features.isNotEmpty ? features : [
            'waiting_list', 'clinical_procedures_form', 'students_evaluation',
            'supervision_groups', 'examined_patients', 'prescription',
            'xray_request', 'assign_patients_to_students',
          ];
        });
  ...
  Future<void> searchPatient() async {
    ...
    try {
      final response = await http.get(Uri.parse('http://localhost:3000/patients'));
      if (response.statusCode == 200) {
        final List<dynamic> data = json.decode(response.body);
  ...
  Future<void> _submitRequest() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('http://localhost:3000/xray_requests'),
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(responseData['message'] ?? 'تم إرسال الطلب بنجاح')));
        _resetForm();
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
